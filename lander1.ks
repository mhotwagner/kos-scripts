...
states:add(state_tracking_high, {
	...
	if height < atmoHeight { set state to state_tracking. set once to true. }
}).
states:add(state_tracking_low, {
	if not once { updateInfo("Falling through atmo"). set once to true. }
